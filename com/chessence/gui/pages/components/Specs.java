...
public class Specs extends JPanel {
    ...
    public static RoundedButton[] specButtons = new RoundedButton[4];
    ...
    public Specs(int W, int H) {
        ...
        for (String item : CreateRoomPanel.SPECTATORS) {
            specButtons[count] = new RoundedButton(item, new Color(0xA05344), new Color(0x8A3D2E), 0);
            specButtons[count].setPreferredSize(new Dimension((int) (size.width * 0.8), size.height / 2));
            specButtons[count].setForeground(new Color(0xfcc9be));
            spec_panel.add(specButtons[count], FlowLayout.TRAILING);
            count++;
        }
    ...
    public static void updateSpecButtons() {
        for(int i=0; i<CreateRoomPanel.SPECTATORS.length; i++){
            specButtons[i].setText(CreateRoomPanel.SPECTATORS[i]);
        }
    }
